...
type Keyring interface {
	...
	//ImportHexAndName(hexStr string, name string)
...
type Importer interface {
	...
	ImportHexAndName(hexStr string, name string) error
}
...
func (ks keystore) ImportHexAndName(hexStr string, name string) error {
	if len(name) == 0 {
		return fmt.Errorf("Name can not be empty\n")
	}
	if len(hexStr) != 64 {
		return fmt.Errorf("hex string must be length 64 (32 bytes)\n")
	}

	derivedPriv, err := hex.DecodeString(hexStr)
	if err != nil {
		return fmt.Errorf("error decoding hex string: %s", hexStr)
	}
	...
	address := sdk.AccAddress(privKey.PubKey().Address())
	if _, err := ks.KeyByAddress(address); err == nil {
		return errors.New("duplicated address created")
	}

	_, err = ks.writeLocalKey(name, privKey)
	return err
}
